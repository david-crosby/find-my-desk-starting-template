...
DeskMate is an AI-powered desk and room booking system designed as a direct alternative to Condeco. It replaces the rigid, manual booking experience with a conversational agent that understands context, learns user preferences, and allocates workspace intelligently — much like a smart hotel concierge.
...
Condeco is clunky. People forget to book. People book and do not show up. Desks sit empty while colleagues cannot find a space. There is no intelligence, no learning, and no feedback loop.
...
Detecting physical presence through Microsoft Places (network / hardware)
...
Scores and ranks available desks against user preferences (location, equipment, proximity to team, anchor days, environmental factors such as noise level and lighting)
...
DeskMate connects to Microsoft Places to detect physical presence without requiring any user action. When a user arrives, their network connection or badge/hardware event is received, the system marks them as checked in and records the visit. If they do not appear by a configurable cut-off time the desk is released back into the pool for same-day re-use.
...
6. Management Intelligence (MI) Dashboard
A lightweight Python web UI (FastAPI + Jinja2 or Streamlit) providing:
...
Technology Stack
LayerTechnologyAI AgentClaude API (claude-sonnet-4-20250514)Backend ServicesPython 3.12, uv package managerAPI LayerFastAPIDatabaseSQLite (via SQLAlchemy)UI / DashboardStreamlit or FastAPI + Jinja2Workplace IntegrationMicrosoft Places APISchedulingAPScheduler (nightly allocation job)NotificationsWebhook / mock email (POC)Configurationpython-dotenv
